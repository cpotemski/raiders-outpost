...
- Keep the import stable at `import "./.next/types/routes.d.ts";` (not `.next/dev/...`).
- If `next dev` rewrites it locally, normalize it back before commit.
- Treat `.next/` output as build artifacts and never commit it.
